# Generador de archivo plano OCN (versión web para GitHub Pages)
...
La plataforma actualmente solo tiene el **módulo OCN** (órdenes de compra a proveedor). El módulo SCC que existía antes fue retirado de la interfaz a pedido del usuario; la lógica sigue documentada en el proyecto de Claude "PLATAFORMA PARA GENERAR SCC Y OCN" por si se necesita reactivar más adelante.

La interfaz usa la misma identidad visual que el resto de herramientas de Compras (mismo logo de Temple en la barra superior, misma paleta de color cobre/crema, mismas tarjetas y tablas que "Seguimiento EA/OS · Ingresos Siesa"), incluyendo el botón **◐ Tema** para alternar entre modo claro y oscuro. El logo va incrustado dentro del propio `index.html` (no depende de ningún archivo de imagen aparte), así que sigue siendo solo 2 archivos para publicar.
...
Entra al link. Si el repositorio trae los maestros compartidos, ya van a estar listos; si no, ve a la pestaña **Maestros** y carga el Excel de proveedores y el de lista de precios (una sola vez por navegador). Luego, en el **Módulo OCN**: sube el archivo de solicitudes de compra, revisa la vista previa, genera, y descarga el archivo resultante. El campo `TERCERO COMPRADOR` está oculto por defecto dentro de "⚙️ Configuración avanzada" — ábrelo solo si necesitas cambiarlo.

## Reglas de negocio ya definidas

- Solo se procesan filas con `Estado = Aprobado`; el resto se descarta.
- `TERCERO COMPRADOR` es fijo (901906381 por defecto, editable en "⚙️ Configuración avanzada").
